...
async function createChore(
  ...
  try {
    ...
    await updateDoc(choreRef, {id: choreRef.id}); //return ((await getDoc(choreRef) ).data())
  } catch (error) {
...
async function assignChorestoUsers(db) {
    // BEATRICE: problem: this goes through every user in the database, not the users in the house, narrow it down to this
  try {
...
async function updateStatus(choreName, db) {
  ...
  const choreDoc = choreSnapshot.docs[0]; //await updateDoc(choreRef, { //    choreStatus: !(choreData.choreStatus) //});
...
async function getXUsersChoreData(userId) {
  ...
  const choreSnapshot = await getDocs(choreQuery); //const justChoreNamesArray = choreSnapshot.data(); //const justNames = justChoreNamesArray.name;
